...
public class SortedArrayList<T extends Comparable<T>> implements SortedListInterface<T> {
    ...
    public boolean remove(T anEntry) {
        if (this.numberOfEntries == 0) {
            ...
            int position = 0;
            while (position < numberOfEntries && array[position].compareTo(anEntry) <= 0) {
                if (array[position].equals(anEntry)) {
    ...
    public boolean contains(T anEntry) {
        return BinarySearch(0, this.numberOfEntries - 1, anEntry);
    }
    ...
    private boolean BinarySearch(int first, int last, T desiredItem) {
        ...
        if (first > last) {
            found = false;
        } else if (desiredItem.equals(this.array[mid])) {
            found = true;
        } else if (desiredItem.compareTo(this.array[mid]) < 0) {
