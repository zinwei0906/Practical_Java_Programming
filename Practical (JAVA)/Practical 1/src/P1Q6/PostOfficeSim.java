...
//    private Queue<Customer> q = new LinkedList<Customer>();
    ...
    private class CounterListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                Customer c = q.poll();
                c.setServeTime(new GregorianCalendar());
                int CounterNumber = Character.getNumericValue(e.getActionCommand().charAt(counterStr.length()));
                c.setCounter(CounterNumber);
                updateDisplay(c);
                announceNumber(c);
                jtfRow1.setText(currentNo + " " + callString + " " + CounterNumber);

                serviceList.add(c);
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(null, "The Take Number Until " + currentNo + " Only", "ERROR", JOptionPane.ERROR_MESSAGE);
            }
    ...
    private void jbtReportActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtReportActionPerformed
        ...
        str += String.format("%-5s %-10s %-20s %-22s %-15s %-15s\n",
                "No", "Seq. No", "Arr. Time (ms)", "Serve Time(ms)", "Counter", "Waiting Time(s)");

        int customerNumber = 1;
        int wattingTime = 0;
        while (customerNumber <= serviceList.size()) {
            Customer cust = serviceList.get(customerNumber - 1);
            str += String.format(" " + customerNumber + " .      " + cust.toString() + "   " + cust.getWaitingTime() + "\n");
            customerNumber++;
            wattingTime += cust.getWaitingTime();
        }
        customerNumber--;
        str += String.format("\nTotal Customer Served : " + customerNumber);
        double averageTime = wattingTime / customerNumber;
        str += String.format("\nAverage Waitting Time : " + averageTime + "s");
        Font reportFont = new Font("Arial", Font.BOLD, 20);
    ...
    private void jbtTakeNumberActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbtTakeNumberActionPerformed
        ...
        Customer c = new Customer(nextNumber);
        q.offer(c);
        nextNumber++;
